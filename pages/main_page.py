...
class MainPage(BasePage):
    ...
    QUESTION = lambda number: (By.XPATH,
                               f".//div[@id='accordion__heading-{number}']")
    ANSWER = lambda number: (By.XPATH,
                             f".//div[@id='accordion__panel-{number}']")
    ORDER_BUTTON = lambda number: (By.XPATH,
                                   f"(.//button[text()='Заказать'])[{number}]")
    MAIN_TITLE = (By.XPATH, ".//div[contains(@class, 'Home_Header')]")

    def go_to_order_scooter(self, number):
        self.click(MainPage.ORDER_BUTTON(number))
    ...
    def has_expected_main_title(self):
        return (data.MainPageData.MAIN_TITLE_H1 in
                self.wait_element_located(MainPage.MAIN_TITLE).text)
